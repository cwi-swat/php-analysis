...
module lang::php::analysis::SimpleAliasRef
...
import experiments::references::SetRef;
...
data AAInfo 
	...
	| aainfo(set[str] definedClasses, InheritanceGraph ig, FieldsRel definedFields, MethodsRel definedMethods, Reference abstractStore, map[str,node] signatures, rel[str,str] referencePairs, set[str] unknownMethods, rel[str,str] targetFields, rel[str,str] classAliases, rel[str,str] methodAliases)
	;
...
public AAInfo calculateAliases(node scr) {
	...
	if (script(bs) := scr) {
		...
		addItemsToSet(res, getInitializingAssignments(aaInfo, "global", vars, gbody));
		...
		for (f:method(sig:signature(_,_,_,_,_,_,_,_,method_name(mn),parameters(fpl)),body(b)) <- bs) {
			...
			addItemsToSet(res,getInitializingAssignments(aaInfo, "global::<mn>", vars, b));
		...
		for (class_def(_,_,class_name(cn),_,_,members(ml)) <- bs, f:method(sig:signature(_,_,_,_,_,_,_,_,method_name(mn),parameters(fpl)),body(b)) <- ml) {
			vars = { vn | bi <- b, /variable_name(vn) <- bi } + { vn | name(variable_name(vn),_) <- fpl };
			addItemsToSet(res,getInitializingAssignments(aaInfo, "<cn>::<mn>", vars, b));
...
public AAInfo propagate(AAInfo aaInfo, str namePrefix, set[str] vars, list[node] body) {
	...
	rel[str,MemItem] deepAssign(str target, str base, str item) {
		...
		res = res + { target } * projectSet(aaInfo.abstractStore, {"<base>::<item>"}, #set[MemItem]);
