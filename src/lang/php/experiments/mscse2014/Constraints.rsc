...
private set[Constraint] deprecatedgetConstraints(Expr expr, M3 m3)
{
	set[Constraint] constraints = {};
	...
	switch(expr) {
		case assign(Expr assignTo, Expr assignExpr): {
			...
			constraints += getConstraints(assignTo, m3);
			constraints += getConstraints(assignExpr, m3);
		}
		
		case v:var(NameOrExpr varName): {
			// check for variable variable...
			if (name(_) := varName) {
				constraints += { subtyp(typeOf(v@at), \any()) };	
			} else if (expr(_) := varName) {
				// do not go deeper into this expression.
				//constraints += { subtyp(typeOf(var), typeOf(v@at)) };	
				println("Handle Variable Variable! :: <v>");
			}
			constraints += {};	
		}
		
		case s:scalar(Scalar scalarVal): 
			constraints += { eq(typeOf(s@at), getTypeOfExpr(s, m3, constraints)) };
				
		
		case b:binaryOperation(Expr left, Expr right, Op operation): {
			//switch(operation) {
//| bitwiseAnd() 
//| bitwiseOr() 
//| bitwiseXor() 
...
//| identical() ;
		...
		case u:unaryOperation(Expr operand, Op operation): {
			switch (operation) {
				case bitwiseNot():	constraints += { eq(typeOf(u@at), getTypeOfExpr(operand, m3, constraints)) };
				case unaryPlus():	constraints += { eq(typeOf(u@at), getTypeOfExpr(operand, m3, constraints)) };
				case unaryMinus():	constraints += { eq(typeOf(u@at), getTypeOfExpr(operand, m3, constraints)) };
			}
	...
	//x	assign(Expr assignTo, Expr assignExpr)
	...
	//x scalar(Scalar scalarVal)
	//x var(NameOrExpr varName)	
